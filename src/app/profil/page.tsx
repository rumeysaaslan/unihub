...
import { Item, Selection } from '@/components/selection'


function Profile() {
  const [selectedCity, setSelectedCity] = useState<City>()
  const [selectedkyk, setSelectedKyk] = useState<Item>()
  ...
  return (
    <div style={{ display: "flex", flexDirection: "row", backgroundColor: '#efe8dd' }} className='space-x-30 justify-center  h-screen'>
      <Card className="w-[350px]">
        <CardHeader>
          <CardTitle style={{color:'#ff3131'}}>Profil</CardTitle>
        ...
        <div className='flex flex-col space-y-3 ml-5'>
          ...
          <div>
            <Selection title='kyk yurt' selectedItem={selectedkyk} setSelectedItem={setSelectedKyk} getItems={async () => {
              const res = await fetch("https://cms.recepdmr.dev/items/dormitories?fields=id,name&filter[province_id]=5");
